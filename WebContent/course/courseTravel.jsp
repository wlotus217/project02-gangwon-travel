				...
				<c:if test="${ch.id eq 'admin' }">
					<div class="row border-bottom ">
						<form class="form-inline"
							action="${pageContext.request.contextPath }/course/course_write.jsp">
								...
								<c:forEach var="vo" items="${list }">
									<div class="col">
										<div class="card my-2" style="width: 18rem; height: 23rem;">
											<a href="${pageContext.request.contextPath }/controller3?type=courseView&idx=${vo.idx}&cPage=${pvo.nowPage}">
												<img
						...
						<ul class="pagination justify-content-center">
							...
							<c:if test="${pvo.beginPage != 1 }">
								<li class="page-item"><a class="page-link"
									href="${pageContext.request.contextPath }/controller3?type=courseTravel&cPage=${pvo.beginPage - 1 }">이전으로</a>
								</li>
							</c:if>
							...
							<c:forEach var="pageNo" begin="${pvo.beginPage }"
								...
								<c:if test="${pageNo != pvo.nowPage }">
									<li class="page-item"><a class="page-link"
										href="${pageContext.request.contextPath }/controller3?type=courseTravel&cPage=${pageNo }">${pageNo }</a></li>
								</c:if>
							</c:forEach>
							...
							<c:if test="${pvo.endPage < pvo.totalPage }">
								<li><a class="page-link"
									href="${pageContext.request.contextPath }/controller3?type=courseTravel&cPage=${pvo.endPage + 1 }">다음으로</a>
								</li>
							</c:if>
							<c:if test="${pvo.endPage >= pvo.totalPage }">
