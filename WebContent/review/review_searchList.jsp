...
<script>
	//전체목록으로 돌어가기
	function list_go() {
		location.href = "${pageContext.request.contextPath }/controller3?type=reviewTravel"
	}
</script>
</head>
<body>
				...
				<div class="row p-1 my-3 border">
					<div class="col-8">
						<p>[${searchKeyword }] 으로
							...
							검색결과 총 ${pvo.totalRecord}건 페이지 ${pvo.nowPage}/${pvo.totalPage}
						</p>
					</div>
					<div class="col-4">
								...
								<c:forEach var="vo" items="${list }">
									<div class="col">
										<div class="card my-2" style="width: 18rem; height: 23rem;">
											<a href="${pageContext.request.contextPath }/controller3?type=reviewView&idx=${vo.idx}&cPage=${pvo.nowPage}&search=${search}&searchKeyword=${searchKeyword}"">
												<img
						...
						<ul class="pagination justify-content-center">
							...
							<c:if test="${pvo.beginPage != 1 }">
								<li class="page-item"><a class="page-link"
									href="${pageContext.request.contextPath }/controller3?type=reviewSearch&search=${search }&searchKeyword=${searchKeyword }&cPage=${pvo.beginPage - 1 }">이전으로</a>
								</li>
							</c:if>
							...
							<c:forEach var="pageNo" begin="${pvo.beginPage }"
								...
								<c:if test="${pageNo != pvo.nowPage }">
									<li class="page-item"><a class="page-link"
										href="${pageContext.request.contextPath }/controller3?type=reviewSearch&search=${search }&searchKeyword=${searchKeyword }&cPage=${pageNo }">${pageNo }</a></li>
								</c:if>
							</c:forEach>
							...
							<c:if test="${pvo.endPage < pvo.totalPage }">
								<li><a class="page-link"
									href="${pageContext.request.contextPath }/controller3?type=reviewSearch&search=${search }&searchKeyword=${searchKeyword }&cPage=${pvo.endPage + 1 }">다음으로</a>
								</li>
							</c:if>
							<c:if test="${pvo.endPage >= pvo.totalPage }">
