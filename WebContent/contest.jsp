...
<div class="col-10">
	...
	<div id="contest">
		...
		<c:otherwise>
			<c:forEach var="contvo" items="${contlist }">
			<div style="flex-wrap: wrap; margin: 20px 20px 20px 0px; display: inline-block; border: 1px solid black;">
					<img src="imgs/${contvo.picName }" style="width: 400px; height: 350px; border: 1px solid black;"><br>
					<a href="contestview.jsp?picNum=${contvo.picNum }&cPage=${pvo.nowPage}" style="font-family: 'Jua', sans-serif; color: black; margin-left: 10px;">${contvo.picTitle }</a>
			</div>
			</c:forEach>
		</c:otherwise>
	...
	<div>
				...
				<c:if test="${pvo.beginPage != 1 }">
					<li>
						<a href="controller4?type=contestlist&cPage=${pvo.beginPage - 1 }">이전으로</a>
					</li>
				</c:if>
				...
				<c:if test="${pageNo != pvo.nowPage }">
					<li><a href="controller4?type=contestlist&cPage=${pageNo }">${pageNo }</a></li>
				</c:if>	
				</c:forEach>
				...
				<c:if test="${pvo.endPage < pvo.totalPage }">
					<li>
						<a href="controller4?type=contestlist&cPage=${pvo.endPage + 1 }">다음으로</a>
					</li>
				</c:if>
				<c:if test="${pvo.endPage >= pvo.totalPage }">
