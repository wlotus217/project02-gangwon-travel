	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-10">
				...
				<div class="row p-1 my-2 border">
					...
					<div class="col-8">
						<form class="form-inline float-right"
							action="${pageContext.request.contextPath }/controller3?type=themeSearch"
							method="post">
				...
				<!-- 새글등록 버튼 ( id가 admin 일때만 보이게 하기) -->
				<c:if test="${ch.id eq 'admin' }">
					<div class="row border-bottom">
						<form class="form-inline"
							action="${pageContext.request.contextPath }/theme/theme_write.jsp">
							<input class="btn btn-secondary m-2" type="submit" value="새글 등록">
						</form>
					</div>
				</c:if>
						...
						<c:otherwise>
							<div class="row row-cols-3 text-center">
								<c:forEach var="vo" items="${list }">
									<div class="col">
										<div class="card my-2 mr-3" style="width: 18rem; height: 23rem;">
											<a href="${pageContext.request.contextPath }/controller3?type=themeView&idx=${vo.idx}&cPage=${pvo.nowPage}">
												<img
						...
						<ul class="pagination justify-content-center">
							...
							<c:if test="${pvo.beginPage != 1 }">
								<li class="page-item"><a class="page-link"
									href="${pageContext.request.contextPath }/controller3?type=themeTravel&cPage=${pvo.beginPage - 1 }">이전으로</a>
								</li>
							</c:if>
							...
							<c:forEach var="pageNo" begin="${pvo.beginPage }"
								...
								<c:if test="${pageNo != pvo.nowPage }">
									<li class="page-item"><a class="page-link"
										href="${pageContext.request.contextPath }/controller3?type=themeTravel&cPage=${pageNo }">${pageNo }</a></li>
								</c:if>
							</c:forEach>
							...
							<c:if test="${pvo.endPage < pvo.totalPage }">
								<li><a class="page-link"
									href="${pageContext.request.contextPath }/controller3?type=themeTravel&cPage=${pvo.endPage + 1 }">다음으로</a>
								</li>
							</c:if>
							<c:if test="${pvo.endPage >= pvo.totalPage }">
