...
<script>
	//전체목록으로 돌어가기
	function list_go() {
		location.href = "${pageContext.request.contextPath }/controller3?type=themeTravel"
	}
</script>
</head>
<body>
								...
								<c:forEach var="vo" items="${list }">
									<div class="col">
										<div class="card my-2" style="width: 18rem; height: 23rem;">
											<a href="${pageContext.request.contextPath }/controller3?type=themeView&idx=${vo.idx}&cPage=${pvo.nowPage}&search=${search}&searchKeyword=${searchKeyword}">
												<img
						...
						<ul class="pagination justify-content-center">
							...
							<c:if test="${pvo.beginPage != 1 }">
								<li class="page-item"><a class="page-link"
									href="${pageContext.request.contextPath }/controller3?type=themeSearch&search=${search }&searchKeyword=${searchKeyword }&cPage=${pvo.beginPage - 1 }">이전으로</a>
								</li>
							</c:if>
							...
							<c:forEach var="pageNo" begin="${pvo.beginPage }"
								...
								<c:if test="${pageNo != pvo.nowPage }">
									<li class="page-item"><a class="page-link"
										href="${pageContext.request.contextPath }/controller3?type=themeSearch&search=${search }&searchKeyword=${searchKeyword }&cPage=${pageNo }">${pageNo }</a></li>
								</c:if>
							</c:forEach>
							...
							<c:if test="${pvo.endPage < pvo.totalPage }">
								<li><a class="page-link"
									href="${pageContext.request.contextPath }/controller3?type=themeSearch&search=${search }&searchKeyword=${searchKeyword }&cPage=${pvo.endPage + 1 }">다음으로</a>
								</li>
							</c:if>
							<c:if test="${pvo.endPage >= pvo.totalPage }">
