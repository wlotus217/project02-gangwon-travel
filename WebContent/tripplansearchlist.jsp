		...
		<table style="font-family: 'Jua', sans-serif;">
			...
			<c:otherwise>
				...
				<tr>
					...
					<td>${planvo.regdate.substring(0,10) }</td>
				</tr>
				</c:forEach>
			</c:otherwise>
			</c:choose>	
			...
			<tr>
				<td colspan="6">
					...
					<c:if test="${pvo.beginPage != 1 }">
						<li>
							<a href="controller4?type=planlistsearch&cPage=${pvo.beginPage - 1 }">이전으로</a>
						</li>
					</c:if>
					...
					<c:if test="${pageNo != pvo.nowPage }">
						<li>
							<a href="controller4?type=planlistsearch&cPage=${pageNo }&triptitle=${triptitle }">${pageNo }</a>
					</li>
					</c:if>	
					</c:forEach>
					...
					<c:if test="${pvo.endPage < pvo.totalPage }">
						<li>
							<a href="controller4?type=planlistsearch&cPage=${pvo.endPage + 1 }">다음으로</a>
						</li>
					</c:if>
					<c:if test="${pvo.endPage >= pvo.totalPage }">
					...
					<li>
						<a href="controller4?type=planlist">전체목록보기</a>
					</li>	
					</ol>
				</td>
			</tr>
