...
<div class="col-10">
	...
	<table style="font-family: 'Jua', sans-serif;">
		...
		<c:otherwise>
			<c:forEach var="infovo" items="${infolist }">
			<tr>
				<td>${infovo.infonum }</td>
				<td>${infovo.info }</td>
				<td>${infovo.city }</td>
				<td>${infovo.cityphone }</td>
				<td>${infovo.address }</td>
			</tr>
			</c:forEach>
		</c:otherwise>
		</c:choose>	
		...
		<tfoot>
			<tr>
				<td colspan="6">
					...
					<c:if test="${pvo.beginPage != 1 }">
						<li>
							<a href="controller4?type=infolist&cPage=${pvo.beginPage - 1 }">이전으로</a>
						</li>
					</c:if>
					...
					<c:if test="${pageNo != pvo.nowPage }">
						<li><a href="controller4?type=infolist&cPage=${pageNo }">${pageNo }</a></li>
					</c:if>	
					</c:forEach>
					...
					<c:if test="${pvo.endPage < pvo.totalPage }">
						<li>
							<a href="controller4?type=infolist&cPage=${pvo.endPage + 1 }">다음으로</a>
						</li>
					</c:if>
					<c:if test="${pvo.endPage >= pvo.totalPage }">
